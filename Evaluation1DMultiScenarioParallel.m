function PopulationUpdated = Evaluation1DMultiScenarioParallel(Population,Scenario,objType)
N = length(Population);
...
for  IndIdx = 1 : N
    ...
    parfor i = 1 : length(Scenario)
        [Jt(i,1),Jc(i,1),Jv(i,1)] =...
            EvaluateFIS(FIS,Scenario(i).SystemParameters,Scenario(i).ArenaParameters,0);
        if objType == 1
            F(i,1) = Jt(i,1);
        elseif objType == 2
            F(i,1) = Jc(i,1);
